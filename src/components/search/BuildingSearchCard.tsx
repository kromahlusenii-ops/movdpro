...
import { MapPin, Home, Star, Users, Building2 } from 'lucide-react'
...
export function BuildingSearchCard({ building, className }: BuildingSearchCardProps) {
  ...
  return (
    <Link
      ...
      className={cn(
        'group flex gap-4 bg-background rounded-xl border p-4 hover:border-foreground/20 hover:shadow-sm transition-all',
        className
      )}
    >
      {/* Photo */}
          ...
          <div className="flex items-center gap-2 mb-1">
            ...
            {building.rating && (
              <div className="flex items-center gap-1 ml-auto">
                ...
                <span className="text-sm font-medium">{building.rating.toFixed(1)}</span>
              </div>
            )}
          </div>
