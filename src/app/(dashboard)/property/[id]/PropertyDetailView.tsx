...
import type { BuildingDetailData } from '@/lib/detail-data'
...
interface PropertyDetailViewProps {
  building: BuildingDetailData
  fieldEdits: Record<string, FieldEditRecord>
  clients: DetailClient[]
}

export function PropertyDetailView({ building, fieldEdits, clients }: PropertyDetailViewProps) {
    ...
    <div className="p-4 md:p-8">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  {building.specialsUpdatedAt && (
                    <p className="text-emerald-600/70 dark:text-emerald-400/70 text-xs mt-2">
                      Last verified: {new Date(building.specialsUpdatedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                    </p>
                  )}
                </div>
              </div>
          ...
          {/* Fees & Policies */}
          {(building.adminFee || building.applicationFee || building.petDeposit || building.maxPets || building.parkingFee) && (
            <div className="bg-background rounded-xl border">
              <div className="p-3 md:p-4 border-b">
                <h2 className="font-semibold text-sm md:text-base">Fees & Policies</h2>
              </div>
              <div className="p-3 md:p-4">
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                    ...
                    <div className="space-y-1.5 text-sm">
                      {building.adminFee && (
                        <div className="flex justify-between gap-4">
                          <span className="text-muted-foreground flex-shrink-0">Admin Fee</span>
                          <span className="font-medium text-right">${building.adminFee}</span>
                        </div>
                      )}
                      {building.applicationFee && (
                        <div className="flex justify-between gap-4">
                          <span className="text-muted-foreground flex-shrink-0">App Fee</span>
                          <span className="font-medium text-right">${building.applicationFee}/person</span>
                        </div>
                      )}
                      {building.incomeRequirement && (
                        <div className="flex justify-between gap-4">
                          <span className="text-muted-foreground flex-shrink-0">Income Req</span>
                          <span className="font-medium text-right">{building.incomeRequirement}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  {/* Pet Policy Card */}
                  {(building.petDeposit || building.petRent || building.maxPets) && (
                    <div className="space-y-2">
                      <div className="flex items-center gap-2 text-sm font-medium mb-2">
                      ...
                      <div className="space-y-1.5 text-sm">
                        {building.petDeposit && (
                          <div className="flex justify-between gap-4">
                            <span className="text-muted-foreground flex-shrink-0">Deposit</span>
                            <span className="font-medium text-right">{building.petDeposit}</span>
                          </div>
                        )}
                        {building.petRent && (
                          <div className="flex justify-between gap-4">
                            <span className="text-muted-foreground flex-shrink-0">Monthly</span>
                            <span className="font-medium text-right">{building.petRent}</span>
                          </div>
                        )}
                        {building.petFeeNonrefundable && (
                          <div className="flex justify-between gap-4">
                            <span className="text-muted-foreground flex-shrink-0">Pet Fee</span>
                            <span className="font-medium text-right">{building.petFeeNonrefundable}</span>
                          </div>
                        )}
                        {building.petWeightLimit && (
                          <div className="flex justify-between gap-4">
                            <span className="text-muted-foreground flex-shrink-0">Weight</span>
                            <span className="font-medium text-right">{building.petWeightLimit}</span>
                          </div>
                        )}
                        {building.maxPets && (
                          <div className="flex justify-between gap-4">
                            <span className="text-muted-foreground flex-shrink-0">Max Pets</span>
                            <span className="font-medium text-right">{building.maxPets}</span>
                          </div>
                        )}
                        {building.petBreedRestrictions && building.petBreedRestrictions !== 'None' && (
                          <div className="pt-1.5 mt-1.5 border-t border-dashed">
                            <div className="flex items-center gap-1.5 text-amber-600">
                              <AlertCircle className="w-3.5 h-3.5 flex-shrink-0" />
                              <span className="font-medium text-xs">
                                {building.petBreedRestrictions === 'Yes'
                                  ? 'Breed restrictions apply'
                                  : building.petBreedRestrictions.replace('Yes ', '').replace('(', '').replace(')', '')}
                              </span>
                            </div>
                          </div>
                        )}
                  ...
                  {building.parkingFee && (
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {building.parkingFee.split(', ').map((option, i) => (
                      ...
                      {building.utilitiesIncluded && (
                        <div className="mt-2 pt-2 border-t">
                          ...
                          <span className="text-xs font-medium">{building.utilitiesIncluded}</span>
                        </div>
                      )}
                    </div>
                  )}
                ...
                {/* Rent Ranges */}
                {(building.rentRangeStudio || building.rentRange1br || building.rentRange2br || building.rentRange3br) && (
                  <div className="mt-4 pt-4 border-t">
                    <div className="flex items-center gap-2 text-sm font-medium mb-3">
                      <FileText className="w-4 h-4 text-muted-foreground" />
                      <span>Rent Ranges by Floor Plan</span>
                    </div>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-sm">
                      {building.rentRangeStudio && (
                        <div className="p-2 rounded-lg bg-muted/50">
                          <span className="text-xs text-muted-foreground block">Studio</span>
                          <span className="font-medium">{building.rentRangeStudio}</span>
                        </div>
                      )}
                      {building.rentRange1br && (
                        <div className="p-2 rounded-lg bg-muted/50">
                          <span className="text-xs text-muted-foreground block">1 BR</span>
                          <span className="font-medium">{building.rentRange1br}</span>
                        </div>
                      )}
                      {building.rentRange2br && (
                        <div className="p-2 rounded-lg bg-muted/50">
                          <span className="text-xs text-muted-foreground block">2 BR</span>
                          <span className="font-medium">{building.rentRange2br}</span>
                        </div>
                      )}
                      {building.rentRange3br && (
                        <div className="p-2 rounded-lg bg-muted/50">
                          <span className="text-xs text-muted-foreground block">3 BR</span>
                          <span className="font-medium">{building.rentRange3br}</span>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {building.additionalProvisions && (
                  <div className="mt-4 pt-4 border-t">
                    <div className="flex items-start gap-2">
                      <AlertCircle className="w-4 h-4 text-muted-foreground mt-0.5 flex-shrink-0" />
                      <p className="text-sm text-muted-foreground">{building.additionalProvisions}</p>
                    </div>
                  </div>
                )}
              </div>
